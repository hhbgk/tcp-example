/**
 * Description:
 * Author:created by bob on 17-12-21.
 */
//
...
{
    tcp_server_t *tcpServer = &tcp_server;
    tcpServer->port = jport;
    tcp_server_set_error_callback(tcpServer, on_error);
    tcp_server_set_state_changed_callback(tcpServer, on_state_changed);
    tcp_server_set_frame_received_callback(tcpServer, on_frame_received);
    return (jboolean) (tcp_server_create(tcpServer) != 0 ? JNI_FALSE : JNI_TRUE);
}
static jboolean jni_close_client(JNIEnv *env, jobject thiz)
{
    tcp_server_t *tcpServer = &tcp_server;
    return (jboolean) (tcp_server_close_client(tcpServer) != 0 ? JNI_FALSE : JNI_TRUE);
}
static jboolean jni_close(JNIEnv *env, jobject thiz)
{
    tcp_server_t *tcpServer = &tcp_server;
    return (jboolean) (tcp_server_close(tcpServer) != 0 ? JNI_FALSE : JNI_TRUE);
}
static jboolean jni_is_receiving(JNIEnv *env, jobject thiz)
{
    return (jboolean) (tcp_server_is_receiving() == 0 ? JNI_TRUE : JNI_FALSE);
}
static jboolean jni_set_frame_rate (JNIEnv *env, jobject thiz, jint jFrameRate)
{
    logi(tag, "%s:%d", __func__, jFrameRate);
    tcp_server_t *tcpServer = &tcp_server;
    tcp_server_set_fps(tcpServer, (uint32_t) jFrameRate);
    return JNI_TRUE;
}
static jboolean jni_set_sample_rate(JNIEnv *env, jobject thiz, jint jSampleRate)
{
    ...
    tcp_server_t *tcpServer = &tcp_server;
    tcp_server_set_sample_rate(tcpServer, (uint32_t) jSampleRate);
    return JNI_TRUE;
}
static jboolean jni_set_resolution (JNIEnv *env, jobject thiz, jint w, jint h)
{
    ...
    tcp_server_set_resolution(tcpServer, w, h);
    return JNI_TRUE;
}
static jboolean jni_init(JNIEnv *env, jobject thiz)
{
    ...
    if (!on_frame_cb_id || !on_error_cb_id || !on_state_changed_cb_id)
        loge(tag, "The calling class does not implement all necessary interface methods");
    return JNI_TRUE;
}
static JNINativeMethod g_methods[] =
        {
                {"nativeInit", "()Z", (void *) jni_init},
                {"nativeCreateServer", "(I)Z", (void*) jni_create},
                {"nativeCloseClient", "()Z", (void*) jni_close_client},
                {"nativeCloseServer", "()Z", (void*) jni_close},
...
int register_tcp_server(JNIEnv* env)
{
